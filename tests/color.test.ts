...
import { deltaE2000, hexToLab, hexToRgb, labToHex, labToRgb, rgbToLab } from '../src/engine/color/color';

/**
 * The full Sharma, Wu & Dalal (2005) CIEDE2000 test dataset — 34 pairs.
 * Columns: L1 a1 b1 L2 a2 b2 expectedΔE00.
 */
...
describe('CIEDE2000', () => {
  it('matches all 34 Sharma reference pairs to 1e-4', () => {
    for (const [L1, a1, b1, L2, a2, b2, expected] of SHARMA) {
      const got = deltaE2000({ L: L1, a: a1, b: b1 }, { L: L2, a: a2, b: b2 });
      expect(Math.abs(got - expected), `pair (${L1},${a1},${b1})↔(${L2},${a2},${b2})`).toBeLessThan(1e-4);
    }
  });

  it('is symmetric and zero on identical colors', () => {
    const a = { L: 42.1, a: 15.3, b: -8.8 };
    const b = { L: 61.2, a: -4.4, b: 22.2 };
...
describe('sRGB ↔ Lab', () => {
  it('round-trips wood-tone hexes within ±2 per channel', () => {
    const hexes = ['#EBD6AE', '#5F4A3A', '#9C6644', '#6B4071', '#A94E26', '#43352B'];
  ...
  it('hex helpers round-trip', () => {
    expect(labToHex(hexToLab('#EBD6AE')).toUpperCase().length).toBe(7);
  });
});
